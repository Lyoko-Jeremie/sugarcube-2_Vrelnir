...
var Wikifier = (() => { // eslint-disable-line no-unused-vars, no-var
	...
	*******************************************************************************************************************/
	class Wikifier {
		constructor(destination, source, options, passageObj) {
			// console.log('class Wikifier constructor:', destination, source);
			// eslint-disable-next-line no-undef
			if (typeof i18nManager !== 'undefined') {
				// eslint-disable-next-line no-undef,no-param-reassign
				source = i18nManager.typeB.replaceInputStoryScript(source, passageObj.title);
			}
		...
		*/
		static wikifyEval(text, passageObj) {
			const output = document.createDocumentFragment();

			new Wikifier(output, text, undefined, passageObj);
