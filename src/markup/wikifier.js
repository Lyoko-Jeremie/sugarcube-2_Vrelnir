	...
	class Wikifier {
		constructor(destination, source, options, passageObj, passageTitle) {
			if (!passageObj && !passageTitle) {
				console.error('Wikifier.constructor(): No passage title.', passageObj, passageTitle, [source]);
			}
			// console.log('class Wikifier constructor:', destination, source);
			// eslint-disable-next-line no-undef
			if (typeof i18nManager !== 'undefined') {
				// eslint-disable-next-line no-undef,no-param-reassign
				source = i18nManager.typeB.replaceInputStoryScript(source, passageObj ? passageObj.title : passageTitle);
			}
		...
		*/
		static wikifyEval(text, passageObj, passageTitle) {
			if (!passageObj && !passageTitle) {
				console.error('Wikifier.wikifyEval(): No passage title.', passageObj, passageTitle, [text]);
			}
